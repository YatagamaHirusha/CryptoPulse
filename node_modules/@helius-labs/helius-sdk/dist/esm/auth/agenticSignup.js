...
import { executeUpgrade, executeCheckout } from './checkout.js';
import { OPENPAY_PLANS } from './constants.js';
import { isOpenPayPlan, buildEndpoints } from './signupHelpers.js';
import { executeBasicSignup } from './basicSignup.js';
...
async function agenticSignup(options) {
    ...
    if (existingProjects.length > 0) {
        ...
        if (isOpenPayPlan(plan)) {
            const upgradeResult = await executeUpgrade(secretKey, jwt, plan, options.period ?? "monthly", project.id, options.couponCode, userAgent);
...
export { agenticSignup };
//# sourceMappingURL=agenticSignup.js.map
