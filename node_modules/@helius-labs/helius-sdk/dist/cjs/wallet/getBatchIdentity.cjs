 ...
 * ```
 */
const getBatchIdentity = async (apiKey, params, userAgent) => {
    const url = `${utils.BASE_URL}/batch-identity${utils.buildQueryString({ "api-key": apiKey })}`;
    const response = await fetch(url, {
        method: "POST",
        headers: utils.getHeaders(true, userAgent), // Include Content-Type for POST
        body: JSON.stringify(params),
    });
    return utils.handleResponse(response);
};

exports.getBatchIdentity = getBatchIdentity;
//# sourceMappingURL=getBatchIdentity.cjs.map
